...
(*
   Denimo, da moramo vrniti število n, find the shortest list of bills such that their sum equals n.
*)
...
(*
   Denimo, da moramo vrniti število n, for rach bill, let s_b denote the recursively computed shortest list 
   to change n-bill. Take that shortest s_b and add its corresponding bill b to it to change the amount n.
*)
...
let rec bills_greedy n =
  ...
  then []
  else let rec max_le den acc = 
      match den with
      | [] -> acc
      | x :: xs -> if x > n then acc else
          max_le xs x in
